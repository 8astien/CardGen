...
# This is the general PNG template used for cards
...
# creating add_margin function for later use
def add_margin(bckgImage, top, right, bottom, left, color):
...
def initTemplate():
    cardTemplate = Image.open("ressources/GeneralTemplate.png").convert(
        "RGBA")  # This is the general PNG template used for cards
    return cardTemplate
...
def getDamage(editCard):
    if (inputBonusDamage.get() != ""):
        editCard.text((100, 1000), dropDamage.get() + dropDice.get() + "+" + inputBonusDamage.get(), forceColor,
                      font=statFont, anchor='ms', stroke_width=3,
                      stroke_fill=outline)  # Title of the card, big text on top
    else:
        editCard.text((100, 1000), dropDamage.get() + dropDice.get(), forceColor, font=statFont, anchor='ms',
                      stroke_width=3, stroke_fill=outline)  # Title of the card, big text on top


def getMana(editCard):
    editCard.text((685, 1000), inputMana.get(), intelligenceColor, font=statFont, anchor='ms', stroke_width=3,
                  stroke_fill=outline)
...
def editCard(nameSpell, typeSpell, editCard):
    match typeSpell:
        case "Force":
            editCard.text((375, 80), nameSpell, forceColor, font=titleFont, anchor='ms', stroke_width=3,
                          stroke_fill=outline)  # Title of the card, big text on top
            editCard.text((375, 700), typeSpell, forceColor, font=typeFont, anchor='ms', stroke_width=2,
                          stroke_fill=outline)  # Type of the card, below artwork
        case "Agilité":
            editCard.text((375, 80), nameSpell, agilityColor, font=titleFont, anchor='ms', stroke_width=3,
                          stroke_fill=outline)  # Title of the card, big text on top
            editCard.text((375, 700), typeSpell, agilityColor, font=typeFont, anchor='ms', stroke_width=2,
                          stroke_fill=outline)  # Type of the card, below artwork
        case "Intelligence":
            editCard.text((375, 80), nameSpell, intelligenceColor, font=titleFont, anchor='ms', stroke_width=3,
                          stroke_fill=outline)  # Title of the card, big text on top
            editCard.text((375, 700), typeSpell, intelligenceColor, font=typeFont, anchor='ms', stroke_width=2,
                          stroke_fill=outline)  # Type of the card, below artwork
        case "Passif":
            editCard.text((375, 80), nameSpell, passiveColor, font=titleFont, anchor='ms', stroke_width=3,
                          stroke_fill=outline)  # Title of the card, big text on top
            editCard.text((375, 700), typeSpell, passiveColor, font=typeFont, anchor='ms', stroke_width=2,
                          stroke_fill=outline)  # Type of the card, below artwork
